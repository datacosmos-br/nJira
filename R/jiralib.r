...
.jira.fields.map <- function(fields, toAlias = FALSE) {
  ...
  for (fld in fields) {
    if (toAlias == FALSE) {
      if (is.element(fld, df$id)) {
        fieldsNew <- append(fieldsNew, fld)
        ...
        if (length(df[df$name == fld, "id"]) > 1 & length(df[df$name == fld & df$custom == "FALSE", "id"]) == 1) {
          ...
          fieldsNew <- append(fieldsNew, df[df$name == fld, "id"])
        }
      } else {
        .logTrace(paste("jira: Following field doesn't exist in JIRA fields -", fld), pr = FALSE)
      }
    } else {
      if (!is.element(fld, df$id)) {
        fieldsNew <- append(fieldsNew, fld)
      } else {
        fieldsNew <- append(fieldsNew, df[df$id == fld, "name"])
      }
    }
  }
  return(fieldsNew)
...
.jira.searchqry <- function(query, clean = FALSE) {
  ...
  df <- sapply(df, function(clm) {
    lapply(clm, function(x) {
      ...
      if ("name" %in% names(x)) {
        rval <- x[["name"]]
        if ("child.name" %in% names(x)) {
          rval <- paste(rval, x[["child.name"]], sep = " - ")
        }
        return(rval)
      }
      if ("value" %in% names(x)) {
        rval <- x[["value"]]
        if ("child.value" %in% names(x)) {
          rval <- paste(rval, x[["child.value"]], sep = " - ")
        }
        return(rval)
      }
      return(x)
  ...
  if (clean == TRUE) {
    df <- Filter(function(x) !all(x == "NULL"), df)
  }
...
.jira.search.issue <- function(query, startAt = 0, maxresults = NULL, fields = NULL, clean = FALSE) {
  ...
  if (is.null(fields)) {
    fields <- paste(.jira.issues.fields(default = TRUE)$id, collapse = ",")
  } else if (fields == "ALL") {
    fields <- "*all,-comment"
    clean <- TRUE
  } else {
    fields <- paste(.jira.fields.map(unlist(strsplit(fields, ","))), collapse = ",")
  }
